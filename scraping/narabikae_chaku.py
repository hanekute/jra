...
if __name__ == "__main__":
    ...
    try:
        df_before = pd.read_csv('scraping/data/jra_ninki_br.csv', low_memory=False, encoding = "shift-jis")
        ...
        for i in range(18):
            chaku = str(i + 1)
            df_result['着 順'+chaku].fillna(9999, inplace=True)
        ...
        for race in result_array:
            ...
            if n < 2:
                print(race[:, 0])
                print(np.argsort(race[:, 0]))
            race = race[np.argsort(race[:, 0])]
            result_array[n] = race
            n += 1
        ...
        CSV_NAME = "scraping/data/jra_chaku_br.csv"
        #データの保存        
